controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    if (sierra.isHittingTile(CollisionDirection.Bottom) || sierra.isHittingTile(CollisionDirection.Left) || sierra.isHittingTile(CollisionDirection.Right)) {
        sierra.vy = -200
    }
})
controller.left.onEvent(ControllerButtonEvent.Pressed, function () {
    sierra.setImage(leftFacingImg)
})
scene.onOverlapTile(SpriteKind.Player, sprites.dungeon.chestClosed, function (sprite, location) {
    game.over(true)
})
controller.right.onEvent(ControllerButtonEvent.Pressed, function () {
    sierra.setImage(rightFacingImg)
})
...
leftFacingImg = img`
    ...
    . . . . f f f . . . f f . . . . 
    `
sierra = sprites.create(rightFacingImg, SpriteKind.Player)
controller.moveSprite(sierra, 100, 0)
tiles.setTilemap(tilemap`level1`)
scene.cameraFollowSprite(sierra)
tiles.placeOnTile(sierra, tiles.getTileLocation(4, 31))
scene.setBackgroundColor(10)
sierra.ay = 350
game.onUpdate(function () {
    if (sierra.isHittingTile(CollisionDirection.Right) && sierra.vy > 0) {
        sierra.ay = 0
        sierra.vy = 15
        sierra.setImage(rightSwordOutImg)
    } else if (sierra.isHittingTile(CollisionDirection.Left) && sierra.vy > 0) {
        sierra.ay = 0
        sierra.vy = 15
        sierra.setImage(leftSwordOutImg)
    } else {
        sierra.ay = 350
        if (sierra.image == leftSwordOutImg) {
            sierra.setImage(leftFacingImg)
        }
        if (sierra.image == rightSwordOutImg) {
            sierra.setImage(rightFacingImg)
        }
    }
})
